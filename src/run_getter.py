...
for i in range(total_jobs):
    ...
    if skip_file != None:
        ...
        if mode == 'r':
            ...
            subprocess.Popen(cmd, stdout = open('r'+log_file,'w'), stderr = open('r'+error_file,'w'))
        if mode == 'b':
            subprocess.call(['bsub', '-R', 'rusage[mem='+mem_limit+']', '-o', 'b'+log_file, '-e', 'b'+error_file,'-W', time_limit, '-q', queue, cmd])
